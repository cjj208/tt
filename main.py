...
if __name__ == "__main__":
    symbolList = ["US30",'XAU/USD','UKOil','USD/JPY','HKG33']
    symbol = "US30"
                ...
                if history.size != 0:
                    ...
                    if df.iloc[-1]["macd_across"] != df.iloc[-2]["macd_across"]:
                        if df.iloc[-1]["macd_across"] ==1:
                            ...
                            message = " macd上穿 %s%s" % (now, sym)
                            call.append(message)
                            #dingtalk(webhook=dinghook, message="%s" % message)
                            #time.sleep(1)
                        if df.iloc[-1]["macd_across"] == -1:
                            message = " macd下穿 %s%s" % (now, sym)
                            call.append(message)
                    if (df.iloc[-1]["macd"] > 0) & (df.iloc[-2]["macd"] < 0):
                        # 当macd柱子向上穿越到零轴
                        message = "macd柱向上突破零轴 %s%s " % (now, sym)
                        call.append(message)
                    if (df.iloc[-1]["macd"] < 0) & (df.iloc[-2]["macd"] > 0):
                        # 当macd柱子跌破零轴
                        message = "macd柱向下跌破零轴 %s%s " % (now, sym)
                        call.append(message)
